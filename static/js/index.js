// 引入数据
fetch('/static/data/distribution_data.json')
  .then(response => response.json())
  .then(data => {

    // 这里可以访问到 JSON 数据，你可以在这里处理数据
        ...
        series: [
          {
            ...
            data: data.chart4.labels.map((label, index) => ({
              name: label,
              value: data.chart4.data[index]
            }))
          }
        ]
      };
...
(function() {
  ...
  var option = {
    ...
    series: [
      ...
      data: data.chart5.labels.map((label, index) => ({
              name: label,
              value: data.chart5.data[index]}))
      }
    ]
  };
  myChart.setOption(option);
    ...
    series: [
      {
        ...
        data: data.chart6.labels.map((label, index) => ({
          name: label,
          value: data.chart6.data[index]
        }))
      }
    ]
  };
    ...
    series: [
      {
        ...
        data: data.chart3.labels.map((label, index) => ({
          name: label,
          value: data.chart3.data[index]
        }))
      }
    ]
  };
